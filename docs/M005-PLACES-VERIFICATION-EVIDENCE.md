...
> **Purpose:** Historical M005 record of what passed, what was mocked, and how the live provider gate handled credential-blocked status. Active provider instructions are now Goong-only.

---

## Scope

S04 was the verification slice for M005. It packages historical proof that:

1. Mocked backend Places contracts handle failure/ownership/negative states truthfully.
2. Frontend ownership checks and map safe-state proofs are static/offline (no browser-side provider lookup).
3. The live provider verifier either passes with normalized candidates (`RESULT=passed`) or exits as `RESULT=credential_blocked` when credentials are absent/unusable.
...
**Result: 5 passed** — validates that frontend source/tests do not own browser-side provider lookup, and map message catalogs include matching non-empty proof-surface keys.
...
**Nature of proof:** Static/offline. Frontend ownership checks verify that browser-side provider lookups are NOT implemented; all Places resolution is server-side.
...
Historical script: replaced by active `scripts/verify-goong-live.py`.

### How it works

1. Imports the backend Places service directly and constructs a `Settings` instance with a harmless OpenAI placeholder (so missing unrelated OpenAI credentials cannot mask provider credential status).
2. Checks the provider API key for presence, length, and obvious placeholder patterns.
3. If the key looks present: makes one Vietnamese (Hàm Ninh) text search to the configured provider API and validates normalized candidates (place_id, display_name, location or provider map URI).
...
```bash
GOONG_API_KEY= python3 scripts/verify-goong-live.py
```

**Output:**
```
CONFIG={"goong_api_key_status": "missing", "location_bias": {"lat": 10.1794, "lng": 104.0491}, "max_result_count": 5, "query_language": "vi"}
...
```bash
GOONG_API_KEY='fake-secret-do-not-print' python3 scripts/verify-goong-live.py
```
...
**Live `RESULT=passed` was not exercised** because no real the configured provider API key was available. This is expected in CI/autonomous environments; re-run with a valid key to obtain live proof.
...
- API key values (provider, OpenAI, or any provider)
...
|---|---------|--------|-------|
| 1 | `GOONG_API_KEY= python3 scripts/verify-goong-live.py` | ✅ `RESULT=credential_blocked`, exit 0 | Blank key — no provider call |
...
- **Live proof (`RESULT=passed`)** requires a valid the configured provider API key. Re-run the verifier script with `GOONG_API_KEY=<valid key>` to obtain live proof.
