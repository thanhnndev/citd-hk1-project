...
7. [Goong Places/Routes/Map Tiles — Integration Contract](#7-goong-placesroutesmap-tiles--integration-contract)
...
- Goong Goong Places API V2 được tách thành section riêng (Section 6) với đặc tả Field Mask, Circuit Breaker, endpoint mapping.
...
| **Tech Stack** | Logo grid: Next.js 16 / FastAPI / LangGraph / Qdrant / Goong Maps / scikit-learn / RAGAS / Langfuse |
...
| OBJ-02 | Hỗ trợ tìm kiếm địa điểm, tuyến đường thực tế thông qua Goong Places API V2 | Functional |
...
│   │   │   ├── map/                 # MapViewer (Goong Maps JS SDK)
...
│   │   ├── places_service.py        # Goong Goong Places API V2 v1 client
│   │   ├── routes_service.py        # Goong Routes API v2 client
...
| **Goong Map Tiles + Mapbox GL** | current | Browser map rendering with public Goong map tiles key |
...
| **httpx** | pinned in backend requirements | Async HTTP client for Goong REST API V2 calls |
...
### 5.5 Goong Maps Platform

| API | Base URL | Vai trò |
|---|---|---|
| **Goong Places API V2** | `https://rsapi.goong.io` | Place search, autocomplete, and details |
| **Goong Distance Matrix API** | `https://rsapi.goong.io` | Distance and duration metrics |
| **Goong Map Tiles** | Goong style/tile endpoints via Mapbox GL | Frontend map rendering with `NEXT_PUBLIC_GOONG_MAPTILES_KEY` |
...
graph TD
    ...
    QD["Qdrant v1.13.6\nVector DB\nHNSW + Gridstore"]
    PA["Goong Places API V2\nrsapi.goong.io"]
    RT["Goong Distance Matrix\nrsapi.goong.io"]
    ...
    MAPS -->|Goong place search| PA
    MAPS -->|Goong distance matrix| RT
    MAPS --> RR
    RAG --> OG
    RR --> OG
...
## 7. Goong Places/Routes/Map Tiles — Integration Contract

### 7.1 Overview

Goong is the active maps provider. Backend services call Goong REST API V2 for Places and Distance Matrix using server-only `GOONG_API_KEY`; frontend map rendering uses Goong Map Tiles through Mapbox GL with `NEXT_PUBLIC_GOONG_MAPTILES_KEY`. The browser must not read `GOONG_API_KEY`, call Places/Routes directly, or own provider credential validation.

Live provider success is valid only when `python3 scripts/verify-goong-live.py` prints terminal `RESULT=passed`. Missing, fake, or placeholder credentials are reported as `RESULT=credential_blocked` and do not prove live provider success.

### 7.2 Backend Endpoints Used

**Places Text Search**

- Endpoint: `GET https://rsapi.goong.io/Place/AutoComplete` or active Goong Places V2 text-search equivalent used by the backend service.
- Purpose: find candidate places for Vietnamese place queries such as "quan ghe Ham Ninh".
- Credential boundary: `GOONG_API_KEY` is read only by backend configuration and live verifier code.
- Normalized fields: `place_id`, `display_name`, `formatted_address`, `location`, `rating`, `price_level`, `map_uri`.

**Place Details**

- Endpoint: Goong Places V2 detail endpoint for a selected `place_id`.
- Purpose: enrich a candidate after search/autocomplete returns an identifier.
- Normalized fields must remain provider-neutral in app responses; frontend consumes `map_uri`, not provider-specific legacy link names.

**Distance Matrix / Routes**

- Endpoint: Goong Distance Matrix route metrics endpoint.
- Purpose: compute distance and duration from the user location to ranked place candidates.
- Normalized fields: distance meters and duration seconds; route diagnostics must redact credentials and raw upstream payloads.

### 7.3 Frontend Map Tiles Boundary

- Renderer: Mapbox GL style backed by Goong Map Tiles.
- Public credential: `NEXT_PUBLIC_GOONG_MAPTILES_KEY` only.
- Forbidden in browser bundles: `GOONG_API_KEY`, backend Places/Routes credentials, direct Places/Routes fetches, or stale provider-specific map URI fields.
- Browser proof: frontend static contract/build gates verify this boundary; they are not live tile-rendering proof unless a dedicated browser/server tile test records it.

### 7.4 Mapping Fields -> Ensemble Re-ranker Features

| Goong normalized field | Feature | Transformation |
|---|---|---|
| `rating` | `rating` | float [1.0, 5.0], direct |
| `user_rating_count` | `review_count_log` | `log(count + 1)` |
| `price_level` | `price_level` | normalized integer 0-4 when available |
| `is_open_now` | `is_open_now` | bool -> int |
| `location` (lat/lng) | `distance_meters` | Haversine/user-location fallback or Distance Matrix metric |
| `types` / category text | `category_match` | cosine sim(query, category/type text) |
| *(internal metadata DB)* | `local_factor` | not sourced from provider |

### 7.5 Circuit Breaker — Fallback

```mermaid
flowchart TD
    Call["Call Goong Places/Routes"]
    Call -->|success| Process["Normalize results"]
    Call -->|timeout or repeated provider errors| CB["Circuit Breaker
activated"]
    CB --> SQLite["Fallback:
SQLite/internal cache"]
    SQLite --> Process
...
### 8.1 Bài toán Re-ranking

**Vấn đề:** Goong Places API V2 mặc định xếp hạng theo `POPULARITY` — phản ánh lượng review và engagement, có lợi cho chuỗi lớn có ngân sách marketing, bất lợi cho tiểu thương địa phương.
...
### 8.2 Feature Space (6 chiều — rút gọn để khả thi)

| Feature | Type | Nguồn | Tính toán |
|---|---|---|---|
| `rating` | float [1.0–5.0] | Goong Places API V2 | Direct |
| `distance_meters` | float | Haversine(user, place) | Tính từ lat/lng |
| `price_level` | int [0–4] | Goong Places API V2 | Direct |
| `is_open_now` | int {0, 1} | Goong Places API V2 | bool → int |
...
### 8.3 `local_factor` — Nhân tố công bằng địa phương
...
### 8.4 Pre-defined Decision Trees (thay thế trained model)
...
### 8.5 Toán học Bagging — Áp dụng với 3 Pre-defined Trees
...
### 8.6 Toán học Boosting — Hiệu chỉnh tuần tự theo Residual
...
### 8.7 So sánh Bagging vs Boosting trong hệ thống
...
### 8.8 Score Breakdown — Explainability
...
### 9.1 Trục 1 — Reliability (Tính tin cậy)
...
### 9.2 Trục 2 — Bias & Fairness (Xử lý thiên vị)
...
### 9.3 Trục 3 — Robustness (Khả năng chịu lỗi)
...
### 9.4 Trục 4 — Social Impact (Tác động xã hội)
...
### 9.5 Trục 5 — Explainability (Tính minh bạch)
...
| `MapViewer` | `/chat`, `/map` | Goong Map Tiles with Mapbox GL, auto-pin gợi ý |
...
### 11.1 Intent Classification — Semantic Router
...
### 11.2 RAG Agent — Retrieval Pipeline
...
### 12.2 Request Schema (Pydantic v2)
...
### 12.3 Response Schema (Pydantic v2)
...
| Field | Type | Mô tả |
|---|---|---|
| `place_id` | `str` | Goong place identifier |
...
| `map_uri` | `str` | Provider-neutral Goong map deep link |

### 12.4 Observability (Langfuse 4.6.1)
...
### 12.5 Infrastructure Docker Compose
...
| **Field Selection** | Chọn trường dữ liệu địa điểm/tuyến đường cần chuẩn hóa để giảm payload và giữ contract ổn định |
...
| **Goong Places API V2** | Goong REST provider for Vietnamese place search, using server-only `GOONG_API_KEY` and no browser exposure |
